...
import com.miniclock.admin.core.complete.XxlJobCompleter;
		...
		callbackThreadPool = new ThreadPoolExecutor(
				...
				new ThreadFactory() {
					...
					public Thread newThread(Runnable r) {
						return new Thread(r, "sd-job, admin JobLosedMonitorHelper-callbackThreadPool-" + r.hashCode());
					}
				},
				new RejectedExecutionHandler() {
					...
					public void rejectedExecution(Runnable r, ThreadPoolExecutor executor) {
						r.run();
						logger.warn(">>>>>>>>>>> sd-job, callback too fast, match threadpool rejected handler(run now).");
					}
				});
			...
			public void run() {
				...
				while (!toStop) {
					try {
						...
						List<Long> losedJobIds  = SdJobAdminConfig.getAdminConfig().getSdJobLogMapper().findLostJobIds(losedTime);
